...
@Singleton
public class DefaultJavaTestFinder implements JavaTestFinder {
  private static final Logger LOG = LoggerFactory.getLogger(DefaultJavaTestFinder.class);
  ...
  public List<String> findTests(
    ...
    List<String> suite = emptyList();
    switch (context.getContextType()) {
      case FILE:
        suite =
            findTestClassDeclaration(findCompilationUnitByPath(javaProject, context.getFilePath()));
        break;
      case FOLDER:
        suite =
        ...
        break;
      case SET:
        suite = convertClassesPathsToFqns(context.getListOfTestClasses(), javaProject);
        break;
      case PROJECT:
        suite = findClassesInProject(javaProject, methodAnnotation, classAnnotation);
        break;
      case CURSOR_POSITION:
        suite =
            findTestMethodDeclaration(
